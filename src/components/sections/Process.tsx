        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          {steps.map((step, index) => (
            <div key={index} className="relative z-10 flex flex-col items-center text-center group">
              <div className={`h-20 w-20 rounded-3xl ${step.color} text-white flex items-center justify-center mb-8 shadow-xl shadow-${step.color.split('-')[1]}-500/20 group-hover:scale-110 transition-transform duration-500`}>
                <step.icon className="h-10 w-10" />
                <div className="absolute -top-4 -right-4 h-8 w-8 rounded-full bg-white dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 flex items-center justify-center text-xs font-bold text-zinc-400">
                  0{index + 1}
                </div>
              </div>
              <h4 className="text-xl font-bold mb-3 group-hover:text-primary transition-colors">{step.title}</h4>

...
interface Props { projects: Project[]; onUpdate: () => void | Promise<void>; }
...
export default function ProjectsManager({ projects, onUpdate }: Props) {
  ...
  const handleSave = async (p: Project) => {
    if (editing) await updateProject(editing.slug, p); else await addProject(p);
    setFormOpen(false); setEditing(null); await onUpdate();
  };
  const handleDelete = async (slug: string) => { await deleteProject(slug); setConfirmDelete(null); await onUpdate(); };
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm text-white transition-all shrink-0 bg-primary hover:bg-primary/90"
        >
            ...
            >
              <div className="relative aspect-video overflow-hidden bg-zinc-100 border-b border-zinc-100">
                ...
                <div className="absolute top-2 left-2">
                  <span className={`text-[7px] md:text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 md:px-3 md:py-1 rounded-full border shadow-sm backdrop-blur-md ${categoryColors[project.category] ?? "bg-white/90 text-zinc-600 border-zinc-200"}`}>
                    {project.category}
                  </span>
                </div>
              </div>
              ...
              <div className="p-3 md:p-5 flex flex-col flex-1">
                <div className="mb-3 md:mb-4">
                  <h4 className="text-zinc-900 font-semibold text-xs md:text-lg leading-tight mb-0.5 md:mb-1 group-hover:text-primary transition-colors line-clamp-1">
                    {project.title}
                  </h4>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
          <div className="bg-white border border-zinc-200 rounded-2xl p-7 w-full max-w-sm shadow-2xl text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)} className="flex-1 py-2.5 rounded-xl border border-zinc-200 text-zinc-500 font-semibold text-sm">Cancel</button>
              <button onClick={() => handleDelete(confirmDelete)} className="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-medium text-sm">Delete</button>
            </div>
          </div>
        </div>
      )}
